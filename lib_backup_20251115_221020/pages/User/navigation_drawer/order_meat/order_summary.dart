...
class OrderSummaryScreen extends ConsumerStatefulWidget {
  ...
  final DateTime? selectedDate;
  final String? selectedTime;
  final bool isReffererPricingPresent;
  ...
  OrderSummaryScreen(
      ...
      required this.totalPrice,
      required this.selectedDate,
      required this.selectedTime,required this.isReffererPricingPresent})
      : super(key: key);
...
class OrderSummaryScreenState extends ConsumerState<OrderSummaryScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        backgroundColor: widget.isReffererPricingPresent ? Constants.colorMeatCPrimary : Constants.secondary4,
        title: Text(
          'Order Details',
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    Expanded(
                                      child: Text(widget.selectedWeight,
                                          style: TextStyle(fontFamily:
                                          'Encode Sans Condensed',fontSize: 17),
                                          textAlign: TextAlign.end),
                                    ),
                                ...
                                options: FFButtonOptions(
                                  ...
                                  iconPadding:
                                      const EdgeInsetsDirectional.fromSTEB(
                                          0.0, 0.0, 0.0, 0.0),
                                  color: widget.isReffererPricingPresent ? Constants.colorMeatCPrimary : Constants.secondary4,
  ...
  Future<void> addMeatOrder(
      {@required String? getMeatType,
      @required String? getArea,
      @required String? getWeight,
      @required String? getChickenOption,
      @required String? getDeliveryDate,
      ...
      @required String? getUserRemarks}) async {
    try {
      Navigator.pop(context);
...
//todo:- adding transaction inside user's table
        FirebaseFirestore.instance
          ...
          'area': getArea,
          'weight': getWeight,
          'chickenoption': getChickenOption,
          ...
          String? getMeat = getMeatType ?? "N/A";
          String? getWeig = getWeight ?? "N/A";
          String? token = await NotificationService.getDocumentIDsAndData();
          if (token != null) {
            Response? response =
                await NotificationService.postNotificationRequest(
                    token,
                    "Hi Admin,\n$getName Trying to Order $getMeat",
                    "$getMeat of $getWeig\nHurry up, let's Delivery Meat.");
